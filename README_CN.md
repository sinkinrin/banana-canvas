...
节点支持按颜色命名分类、数量统计和一键定位，详见[节点分类](docs/NODE_CATEGORIES.md)。
...
- **无边框桌面窗口**：顶部拖动区与界面同色，提供最小化、最大化/还原和关闭按钮；关闭前等待项目保存，完成后彻底退出。
- **可视化生图工作流**：基于 React Flow 自由摆放、连接和整理提示词与图片节点。
- **多模型对比**：同一提示词与参考图，每个模型生成一张；在画布直接查看整组、缩放和标记最佳方案，支持单独停止任务。
- **结果信息卡**：查看实际尺寸、格式、耗时、请求模型和服务器回报型号/画质，缺失数据明确显示“未记录”。
- **服务器模型列表**：在应用设置中查询、刷新和搜索已保存连接的标准模型列表。
- **多模型支持**：新节点默认使用 Image2，也可切换 Image 2.5 Flare、Image 2.5 Sunburst、Banana 2、Banana 2 Lite 和 Banana Pro。
- **Image 2.5 透明素材**：支持直接生成透明素材，已实测透明 PNG；Flare 侧重快速日常创作，Sunburst 侧重精确编辑。
- **参考图迭代**：上传或粘贴最多 4 张参考图，并可从任意结果继续创作。
- **构图草图**：生成前先画人物位置、动作、箭头和画面关系，草图会作为参考图发送。
- **蒙版局部编辑**：涂抹需要修改的区域，使用 Image2 生成并对比原图与新图。
- **离线智能抠图**：桌面版内置 IS-Net INT8，可在设置里下载和切换其他模型。详见[智能抠图说明](docs/CUTOUT.md)。
- **跨项目提示词库**：保存、搜索、标记并复用常用提示词。
- **本地优先项目**：画布、节点关系与图片资产都保存在自己的设备上。
- **中英文界面**：首次启动跟随系统语言，手动切换后会记住选择。

从 v0.8.0 起支持 Image 2.5。两款模型共用 Image2 连接，在每个创作节点中选择；参考图、蒙版编辑、重新生成和项目保存会保留所选型号。实测中转对 `xhigh` / `max` 请求回报 `low` / `medium`，因此暂未开放这两档。完整实测结果与限制见 [Image 2.5 说明](docs/IMAGE25.md)。
...
浏览项目、整理画布、查看已保存图片和桌面版本地智能抠图，都不需要 API Key。
...
创作节点点击“**多模型对比**”，选择至少两个模型及共用画幅、分辨率。开始生成后直接回到画布，每个模型生成一张，各自可能产生费用；结果并排保留在画布上，保存重开后仍可通过“查看对比”定位整组并查看最佳标记。某个模型失败或单独停止不会丢弃其他结果。详见[多模型对比与结果信息](docs/COMPARISON.md)。
...
- 启动时检查更新默认开启，发现新版本会弹窗，可在“应用设置 → 软件更新”关闭；后台自动下载仍需单独开启，重启安装前仍会询问用户。
