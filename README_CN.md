...
- **多模型对比**：同一提示词与参考图，每个模型生成一张；支持同步缩放、拖动查看和标记最佳方案。
...
创作节点点击“**多模型对比**”，选择至少两个模型及共用画幅、分辨率。每个模型生成一张，各自可能产生费用；结果并排保留在画布上，保存重开后仍可“查看对比”和查看最佳标记。某个模型失败不会丢弃其他结果。详见[多模型对比与结果信息](docs/COMPARISON.md)。
